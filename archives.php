...
if(isset($db)) {
    ...
    if(isset($_GET['created_at'])) {
        ...
        $articlesByDate = $db->getArticlesByDate($created_at);
    }

    if(isset($_GET['category'])) {
        $category = $_GET['category'];
        $articlesByCategory = $db->getArticlesByCategory($category);
    }

    if(isset($_GET['user_name'])) {
        $userName = $_GET['user_name'];
        $articlesByAuthor = $db->getArticlesByAuthor($userName);
    }
...
<div class="page-heading">
    <div class="container">
        <div class="heading-content">
            <?php
            if(isset($_GET['created_at'])) { ?>
                <h1>Posts CREATED AT <em><?php echo date("Y, F", strtotime($_GET['created_at']))?></em></h1>
            <?php } else if(isset($_GET['category'])) {?>
                <h1>Posts of CATEGORY <em><?php echo $_GET['category']?></em></h1>
            <?php } else
                if(isset($_GET['user_name'])) { ?>
                <h1>Posts by <em><?php echo $_GET['user_name']?></em></h1>
            <?php } ?>
        </div>
    </div>
</div>
        ...
        <div class="col-md-3">
            <div class="side-bar">
                ...
                <div class="categories">
                    ...
                    <ul>
                        <?php
                        foreach ($archivesCategories as $archiveCategory) { ?>
